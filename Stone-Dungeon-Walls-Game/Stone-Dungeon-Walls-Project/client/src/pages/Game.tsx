...
export default function Game() {
  ...
  const [combatState, setCombatState] = useState<{ 
    ...
    defending: boolean,
    monsterAnimStates: ('idle' | 'attack' | 'hit' | 'death')[],  // Animation state for each monster
    playerIsAttacking: boolean  // Track if player is attacking
  }>({ active: false, monsters: [], targetIndex: 0, turn: 0, currentCharIndex: 0, turnOrder: [], turnOrderPosition: 0, defending: false, monsterAnimStates: [], playerIsAttacking: false });
  ...
  const move = useCallback((dx: number, dy: number) => {
    ...
    if (tile === TILE_FLOOR && Math.random() < 0.1) {
      ...
      setCombatState({ 
        ...
        turnOrderPosition: 0,
        defending: false,
        monsterAnimStates: monsters.map(() => 'idle' as const),  // Initialize all monsters to idle
  ...
  const monsterTurn = useCallback((updatedMonsters: Monster[], defendingActive: boolean) => {
    ...
    // Process monster attacks sequentially with animations
    let attackDelay = 0;
    sortedMonsters.forEach((monster, monsterIdx) => {
      setTimeout(() => {
        // Find monster's index in original array for animation state
        const actualMonsterIdx = updatedMonsters.findIndex(m => m.id === monster.id);
        
        // Trigger monster attack animation
        setCombatState(prev => {
          const newAnimStates = [...prev.monsterAnimStates];
          newAnimStates[actualMonsterIdx] = 'attack';
          return { ...prev, monsterAnimStates: newAnimStates };
        });
        
        // Wait for attack animation, then apply damage
        setTimeout(() => {
          const aliveMembersNow = newParty.filter(c => c.hp > 0);
          if (aliveMembersNow.length === 0) return;
          ...
          // Return monster to idle
          setCombatState(prev => {
            const newAnimStates = [...prev.monsterAnimStates];
            newAnimStates[actualMonsterIdx] = 'idle';
            return { ...prev, monsterAnimStates: newAnimStates };
          });
          
          // If this is the last monster, finish the turn
          if (monsterIdx === sortedMonsters.length - 1) {
            setTimeout(() => {
        ...
        }, 400); // Wait for attack animation
      }, attackDelay);
      
      attackDelay += 800; // Stagger attacks
    });
  }, [game, log]);

  // Helper function to process combat after ability use (for animation timing)
  const processAfterAbility = useCallback((newMonsters: Monster[], newParty: Player[], isDefending: boolean, targetMonster: Monster, ability: Ability) => {
    setGame(prev => prev ? ({ ...prev, party: newParty }) : null);
    ...
    if (aliveMonsters.length === 0) {
      ...
      setCombatState({ active: false, monsters: [], targetIndex: 0, turn: 0, currentCharIndex: 0, turnOrder: [], turnOrderPosition: 0, defending: false, monsterAnimStates: [], playerIsAttacking: false });
    ...
    // Check if current character's turn is over
    const nextTurnOrderPosition = combatState.turnOrderPosition + 1;
    if (nextTurnOrderPosition >= combatState.turnOrder.length) {
      ...
      setCombatState(prev => ({
        ...
        monsterAnimStates: newMonsters.map(m => m.hp <= 0 ? 'death' as const : 'idle' as const)
      }));
      setTimeout(() => monsterTurn(newMonsters, isDefending), 500);
    } else {
      const nextCharIndex = combatState.turnOrder[nextTurnOrderPosition];
      setCombatState(prev => ({
        ...prev,
        monsters: newMonsters,
        targetIndex: newTargetIndex,
        currentCharIndex: nextCharIndex,
        turnOrderPosition: nextTurnOrderPosition,
        defending: isDefending,
        monsterAnimStates: newMonsters.map(m => m.hp <= 0 ? 'death' as const : 'idle' as const)
      }));
    }
  }, [combatState, log, game]);
  ...
  const useAbility = (ability: Ability, charIndex: number) => {
    ...
    switch (ability.type) {
      case 'attack': {
        ...
        setCombatState(prev => ({ ...prev, playerIsAttacking: true }));
        ...
        setTimeout(() => {
          ...
          const newAnimStates = [...combatState.monsterAnimStates];
          newAnimStates[combatState.targetIndex] = 'hit';
          setCombatState(prev => ({
            ...prev,
            monsterAnimStates: newAnimStates,
            playerIsAttacking: false
          }));
          
          // Return monster to idle after hit animation
          setTimeout(() => {
            const idleStates = [...combatState.monsterAnimStates];
            idleStates[combatState.targetIndex] = newMonsters[combatState.targetIndex].hp <= 0 ? 'death' : 'idle';
            setCombatState(prev => ({ ...prev, monsterAnimStates: idleStates }));
          }, 500);
        }, 300);
        
        // Wait for all animations before continuing
        setTimeout(() => {
          processAfterAbility(newMonsters, newParty, isDefending, targetMonster, ability);
        }, 1000);
        return; // Exit early to wait for animations
      }
      case 'heal': {
                          ...
                          {monster.image ? (
                            <>
                              <AnimatedMonster 
                                ...
                                isAttacking={combatState.monsterAnimStates[idx] === 'attack'}
                                isHit={combatState.monsterAnimStates[idx] === 'hit'}
                                isDead={monster.hp <= 0}
                                isFlying={['Cave Bat', 'Shadow Wisp', 'Harpy', 'Wraith', 'Dragon', 'Gargoyle'].includes(monster.name)}
                                isBoss={['Dragon', 'Lich', 'Demon', 'Death Knight', 'Golem', 'Basilisk'].includes(monster.name)}
                              />
